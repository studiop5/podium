...
import { css, ButtonGroup, clamp, clearChildren, dataIndex, delay, delayMs, getBox, helm, iconSvg, listen, mvmt, dialog, Schedule, strToHash, toast, unlisten } from "./common.js";
...
class LocalSrc extends FileSrc {
  ...
  async putFile(path, name, data, handle = null) {
    if (window.showSaveFilePicker) {
      ...
      link.href = url;
      link.click();
      delayMs(4000, () => URL.revokeObjectURL(url));
...
class CachedSrc extends FileSrc {
    ...
    **/
    ...
    // Support both web app and browser extension contexts
    redirectUri = (typeof chrome !== 'undefined' && chrome.runtime?.id)
      ? `chrome-extension://${chrome.runtime.id}/podauth.html`
      : `${location.origin}/podauth.html`;
    tokens = null;
...
class GDriveSrc extends CachedSrc {
  ...
  // NOTE: Google doesn't provide a usable PKCE flow for browser apps (CORS issues on token endpoint)
  // so we use the implicit grant flow (response_type=token) instead

  async auth() {
    if (this.tokens && this.tokens.expiry > Date.now() / 1000) return Promise.resolve();
    this.tokens = null;
    let state = Math.random().toPrecision(48) + Math.random().toPrecision(48);
    let timeout = (Date.now() / 1000) + this.authTimeout;
    ...
    return new Promise((resolve, reject) => {
      let poll = async () => {
        try {
          ...
          let href = popup.location.href;
          let access_token = this.getQuery(href, "access_token=");
          if(access_token) {
            if(this.getQuery(href, "state=") != state)
              throw new Error("Possible <i>Cross Site Request Forgery</i> attempt blocked.", {cause:"security"});
            let expires_in = parseInt(this.getQuery(href, "expires_in=") || "3600");
            this.tokens = {
          ...
          if ((Date.now() / 1000) > timeout) throw new Error("", {cause: "timeout"});
          delay(1, poll);
        } catch (e) {
          // SecurityError is expected while popup is on Google's domain - keep polling
          if (e.name === 'SecurityError') {
            if ((Date.now() / 1000) > timeout) {
              this.authPopupClose(popup);
              reject(new Error("", {cause: "timeout"}));
            } else {
              delay(1, poll);
            }
          } else {
...
class ODriveSrc extends CachedSrc {
  ...
  // Uses inherited PKCE auth() from CachedSrc - no custom auth needed
